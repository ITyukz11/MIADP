...
const SearchPage: React.FC<{ open: () => void }> = ({ open }) => {
    ...
    return (
        ...
        <table className="min-w-full divide-y divide-gray-200 shadow-lg w-full">
                    ...
                    <tbody className="bg-white divide-y divide-gray-200">
                        {filteredFormData.map((item, index) => (
                            <tr key={index}>
                                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500" contentEditable onBlur={(e) => handleEdit(index, 'subprojectName', e.target.innerText)}>{item.subprojectName}</td>
                                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500" contentEditable onBlur={(e) => handleEdit(index, 'description', e.target.innerText)}>{item.description}</td>
                                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500" contentEditable onBlur={(e) => handleEdit(index, 'region', e.target.innerText)}>{item.region}</td>
                                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500" contentEditable onBlur={(e) => handleEdit(index, 'province', e.target.innerText)}>{item.province}</td>
                                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500" contentEditable onBlur={(e) => handleEdit(index, 'municipality', e.target.innerText)}>{item.municipality}</td>
                                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500" contentEditable onBlur={(e) => handleEdit(index, 'subprojectType', e.target.innerText)}>{item.subprojectType}</td>
                                <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500" contentEditable onBlur={(e) => handleEdit(index, 'projectCost', e.target.innerText)}>{item.projectCost}</td>
